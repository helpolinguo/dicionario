# Soulignements non places

L'auteur souligne ce qu'une imprimerie mettrait en italique : le
domaine, la locution qui porte sa propre definition, le nom
scientifique. L'edition epuree lit ces filets et les rend. Voici ceux
qu'elle n'a pas su placer : le fragment releve ne se retrouve pas tel
quel dans le texte, ou ne couvre que des mots-outils.

**1762 fragments**, sur 9467 articles. La premiere famille est
la seule qui demande un arbitrage : les autres sont des artefacts du
releve des filets, ou le trait deborde ou s'arrete trop tot.

## Ressemble a un qualificatif ou a une locution — 27

| page | mot-vedette | fragment souligne |
|---:|---|---|
| 123 | dronto | `Didus` |
| 135 | emiro | `II. Ti` |
| 140 | eperlano | `Osmerus eperlanus.-` |
| 204 | guindo | `Cerasus` |
| 221 | holoturio | `Holo` |
| 282 | kobayo | `Cavis cobay` |
| 289 | komodoro | `Navaro B` |
| 314 | kreto | `Kreto-kra-` |
| 332 | lavendo | `Plant` |
| 334 | lentisko | `Pistacia` |
| 360 | materio | `I. (filoz` |
| 378 | modlar | `Anke alu-` |
| 392 | nenufaro | `Nuphar, Nymphae` |
| 394 | niktalopo | `Pers` |
| 398 | numulario | `Lysi-` |
| 405 | olivo | `L. olea europaea` |
| 409 | ordonanco | `Ordonanc-` |
| 413 | ovacionar | `Hono` |
| 415 | pajo | `Ludo-` |
| 443 | plano | `Surf` |
| 463 | profitar | `I. E` |
| 485 | reciproka | `Propozici` |
| 514 | saturnali | `Nia-` |
| 517 | sen | `Preppziciono` |
| 523 | shaloto | `EFRS` |
| 527 | signifikar | `Progres` |
| 531 | sinoptika | `Kurso di la filozo-` |

## Fragment de trois lettres ou moins — 544

| page | mot-vedette | fragment souligne |
|---:|---|---|
| 1 | a(d) | `a(d` |
| 3 | aborijena | `pop` |
| 10 | agendo | `uze` |
| 11 | agregar | `rti` |
| 11 | ah! | `ah !` |
| 14 | akromatino | `bla` |
| 15 | akushar | `per` |
| 15 | akuzar | `ala` |
| 15 | alo | `i t` |
| 18 | alineo | `sur` |
| 20 | alpari | `ala` |
| 22 | amazono | `vir` |
| 26 | anakoluto | `Qua` |
| 26 | anakoluto | `est` |
| 27 | anemometro | `o p` |
| 31 | antitezo | `udi` |
| 36 | apta | `Qua` |
| 38 | argano | `ina` |
| 38 | argono | `tim` |
| 43 | asbesto | `qua` |
| 44 | asno | `e q` |
| 45 | aspiratoro | `kor` |
| 50 | autopsiar | `aut` |
| 50 | avan | `van` |
| 52 | aye! | `aye !` |
| 53 | ba! | `ba !` |
| 56 | bandajo | `por` |
| 56 | bandajo | `por` |
| 56 | banderolo | `oft` |
| 58 | barometro | `r p` |
| 58 | baroskopo | `ega` |
| 59 | baskular | `a a` |
| 60 | bazaro | `Lok` |
| 62 | benefico | `ico` |
| 63 | betelo | `dia` |
| 64 | bibliofilo | `rsa` |
| 66 | biologio | `ant` |
| 67 | bizono | `bos` |
| 68 | blua | `pas` |
| 68 | boikotar | `tar` |
| 70 | borso | `aci` |
| 73 | brido | `sas` |
| 74 | «brokoli»-kaulo | `brokoli"-kaulo` |
| 75 | brosar | `ako` |
| 75 | bruta | `ins` |
| 76 | bufro | `lst` |
| 76 | buketo | `e k` |
| 78 | bushelo | `dek` |
| 78 | busolo | `iga` |
| 81 | caro | `olim, ante l9l5` |
| 81 | cefalopodo | `vic` |
| 82 | ceno | `eat` |
| 83 | centifolio | `i q` |
| 86 | chamaro | `ver` |
| 86 | chambelano | `ham` |
| 87 | charniro | `eti` |
| 88 | chifrar | `kas` |
| 88 | chiniono | `i l` |
| 90 | ciklito | `kor` |
| 93 | cirkonspekta | `dic` |
| 93 | cisoido | `end` |
| 94 | citologio | `e l` |
| 97 | deo | `kre` |
| 101 | delikata | `a : di` |
| 105 | despenso | `e o` |
| 107 | dextra | `ter` |
| 107 | dezirar | `ola` |
| 107 | diablo | `dia` |
| 108 | diafana | `rad` |
| 108 | diakilono | `- I` |
| 111 | digramo | `ibe` |
| 113 | dinosaurio | `rin` |
| 113 | dinoterio | `ami` |
| 114 | diptiko | `qua` |
| 114 | diptiko | `qua` |
| 115 | disipar | `dis` |
| 115 | diskriminanto | `c -` |
| 116 | dismo | `aci` |
| 118 | dizastro | `xem` |
| 119 | docila | `vas` |
| 120 | domajar | `min` |
| 121 | dorado | `Co-` |
| 124 | dueto | `i o` |
| 124 | duramatro | `aso` |
| 126 | e(d) | `e(d` |
| 128 | edukar | `e l` |
| 128 | efemerido | `oro` |
| 132 | elevatoro | `mek` |
| 132 | eliminar | `eli` |
| 132 | eliminar | `nar` |
| 136 | emulsar | `ext` |
| 140 | epiciklo | `aji` |
| 140 | epigloto | `ped` |
| 142 | epoleto | `o d` |
| 142 | equinoxo | `io-` |
| 143 | -er- | `-er` |
| 145 | eshafodo | `s d` |
| 145 | eskamotar | `esk` |
| 146 | eskorto | `imp` |
| 146 | espado | `e k` |
| 147 | estafeto | `o q` |
| 149 | etapar | `par` |
| 149 | eterna | `nos` |
| 149 | etmoido | `a o` |
| 152 | exhalar | `org` |
| 152 | exhaustar | `gra` |
| 153 | exodo | `e E` |
| 153 | exotera | `Dic` |
| 155 | extorsar | `gen` |
| 156 | extraordinara | `esa` |
| 157 | fago | `ago` |
| 158 | fakto | `n a` |
| 159 | fanar | `sas` |
| 160 | fantomo | `- I` |
| 160 | farcino | `i e` |
| 161 | farso | `a d` |
| 163 | felina | `mov` |
| 165 | fi! | `fi !` |
| 166 | filamento | `qua` |
| 167 | fileto | `nek` |
| 167 | filio | `der` |
| 171 | fluida | `ist` |
| 172 | fluvio | `ush` |
| 172 | fola | `i s` |
| 172 | folio | `sas` |
| 174 | forejo | `ala` |
| 175 | fortuno | `bon` |
| 175 | foso | `plu` |
| 177 | frako | `dat` |
| 177 | frambo | `mbo` |
| 179 | fremisar | `fre` |
| 179 | frequentar | `oft` |
| 183 | fureto | `cha` |
| 185 | gafo | `nda` |
| 189 | garoto | `str` |
| 190 | genero | `nti` |
| 192 | gibo | `ron` |
| 197 | gluo | `a s` |
| 197 | gnuo | `gnu` |
| 198 | *golfo | `+golfo` |
| 199 | grafika | `des` |
| 200 | grapino | `uan` |
| 201 | grenado | `e d` |
| 202 | griliotalpo | `gri` |
| 203 | grucho | `irm` |
| 203 | grumelo | `pul` |
| 204 | *gulfo | `+gulfo` |
| 204 | *gulfo | `rto` |
| 204 | gulo | `gul` |
| 205 | guvernar | `- I` |
| 208 | harpo | `kal` |
| 209 | he! | `he !` |
| 211 | hem! | `hem !` |
| 212 | herbo | `ovr` |
| 215 | hidrauliko | `hid` |
| 219 | hipopotamo | `vii` |
| 220 | *ho- | `+ho-` |
| 220 | *hobio | `+hobio` |
| 220 | ho! | `ho !` |
| 222 | hop! | `hop !` |
| 223 | hospico | `voy` |
| 224 | hu! | `hu !` |
| 224 | hum! | `hum !` |
| 225 | hura! | `hura !` |
| 227 | -ig- | `kun` |
| 227 | -ig- | `kun` |
| 228 | iktiokolo | `oni` |
| 229 | imbibar | `bar` |
| 229 | imolar | `isi` |
| 230 | impostar | `pub` |
| 233 | indulto | `ene` |
| 233 | indulto | `ind` |
| 233 | indulto | `lto` |
| 233 | infantrio | `rio` |
| 234 | inflexar | `gen` |
| 236 | inkursar | `eso` |
| 236 | inokular | `ulu` |
| 241 | *intrenar | `+intrenar` |
| 241 | introduktar | `oci` |
| 244 | *itemo | `+itemo` |
| 246 | ja | `kan` |
| 246 | jakobino | `agn` |
| 249 | jonko | `alt` |
| 249 | jonquilo | `pro` |
| 249 | jujubo | `n f` |
| 252 | kadencar | `cen` |
| 252 | kadro | `ond` |
| 255 | kaloro | `ino` |
| 256 | kalzo | `nil` |
| 256 | kamalio | `n m` |
| 256 | kamo | `ans` |
| 257 | kampanulo | `ori` |
| 258 | kano | `ero` |
| 259 | kanguruo | `a t` |
| 261 | kapo | `taf` |
| 262 | kaprikorno | `int` |
| 263 | kara | `ara` |
| 265 | kardinala | `pua` |
| 266 | karezar | `mor` |
| 268 | kartilago | `ktu` |
| 269 | kartuzio | `uno` |
| 269 | kaso | `qua` |
| 273 | kaverno | `nat` |
| 273 | kayo | `arg` |
| 273 | kazo | `i r` |
| 276 | kilogrametro | `nec` |
| 277 | kizelguro | `ico` |
| 278 | klavikulo | `Ost` |
| 279 | klepsidro | `tit` |
| 279 | klinar | `nar` |
| 279 | kliniko | `kal` |
| 280 | klipero | `ata` |
| 280 | kloroplasto | `klo` |
| 280 | kloroplasto | `pla` |
| 280 | kloroplasto | `uan` |
| 281 | *kluza | `+kluza` |
| 281 | klozar | `o e` |
| 282 | kodicilo | `qua` |
| 283 | *koino | `+koino` |
| 286 | kolofono | `sur` |
| 286 | kolonio | `oku` |
| 287 | komandar | `ekn` |
| 288 | kombatar | `cus` |
| 289 | komizo | `oya` |
| 289 | komodo | `ita` |
| 289 | kompanio | `abr` |
| 291 | komto | `bel` |
| 295 | kongro | `k l` |
| 296 | konjugar | `Imaginari konjugaa + b i` |
| 296 | konjugar | `a' + b' i` |
| 299 | konsomeo | `oqu` |
| 303 | konvenar | `sar` |
| 304 | konvolvulo | `lva` |
| 305 | korano | `nas` |
| 305 | kordio | `sur` |
| 306 | korimbo | `sas` |
| 307 | korno | `s s` |
| 308 | korpulenta | `ent` |
| 309 | korto | `tri` |
| 309 | korto | `uri` |
| 309 | koruptar | `Des` |
| 310 | kotono | `nun` |
| 311 | krak! | `krak !` |
| 311 | krano | `sub` |
| 312 | krecento | `orm` |
| 314 | krik! | `krik !` |
| 314 | krik-krak | `krik-krak !` |
| 317 | kroniko | `lia` |
| 317 | kronologio | `ala` |
| 318 | krotalo | `tal` |
| 318 | krotono | `m d` |
| 318 | kruda | `kru` |
| 318 | kruda | `rso` |
| 321 | kuliso | `gar` |
| 321 | kultar | `gar` |
| 324 | kurta | `tes` |
| 325 | kuto | `uto` |
| 326 | laborar | `por` |
| 327 | lacerto | `fle` |
| 328 | lamao | `r f` |
| 328 | lano | `e t` |
| 331 | lasta | `+ultima` |
| 335 | *leyo | `+leyo` |
| 337 | ligar | `plu` |
| 341 | litiero | `pal` |
| 342 | lobelio | `lio` |
| 342 | lobelio | `lob` |
| 342 | lobo | `obo` |
| 343 | logiko | `- I` |
| 344 | longa | `lon` |
| 346 | lumar | `lum` |
| 346 | lumbo | `nch` |
| 349 | magazino | `eso` |
| 349 | magnezio | `Simb. kem.: Mg` |
| 350 | maklo | `e l` |
| 351 | malada | `sis` |
| 352 | mamifero | `e l` |
| 352 | mamo | `sim` |
| 352 | mandarino | `lis` |
| 355 | mantenar | `nar` |
| 356 | maro | `cii` |
| 358 | marquizo | `ifi` |
| 358 | marsupialo | `ure` |
| 359 | maskar | `- e` |
| 360 | matadoro | `vas` |
| 360 | materio | `esa` |
| 361 | matura | `gis` |
| 361 | matutino | `o -` |
| 361 | maxilo | `u f` |
| 362 | medalio | `+wera` |
| 362 | mediano | `som` |
| 365 | menhiro | `lta` |
| 366 | mento | `i l` |
| 366 | mercero | `qua` |
| 367 | mestiero | `ual` |
| 368 | metalo | `o m` |
| 369 | metonomazio | `s =` |
| 375 | minoritato | `qua` |
| 375 | minto | `nto` |
| 376 | mirmekofila | `mir` |
| 377 | mixar | `deb` |
| 378 | modesta | `a o` |
| 378 | modo | `III` |
| 378 | modo | `zar` |
| 379 | mokasino | `i l` |
| 379 | molara | `ara` |
| 379 | moloso | `Nun` |
| 379 | moloso | `sar` |
| 379 | molusko | `DEF` |
| 380 | monero | `tot` |
| 380 | monitoro | `sur` |
| 382 | monsinioro | `lio` |
| 382 | monstro | `mi-` |
| 382 | monstro | `tol` |
| 382 | mordar | `oro` |
| 382 | moreno | `aku` |
| 383 | mortero | `por` |
| 383 | mortezo | `o f` |
| 386 | mumio | `u r` |
| 386 | mungar | `mun` |
| 387 | mutilar | `ens` |
| 387 | mutono | `kun` |
| 388 | *muvmento | `+muvmento` |
| 390 | navo | `ang` |
| 391 | nekromanciar | `nar` |
| 392 | neolamarckismo | `n l` |
| 393 | *nexta | `+nexta` |
| 393 | neta | `kti` |
| 394 | nirvana | `nirvana".(filoz. Indiana` |
| 395 | no | `ver` |
| 397 | nu! | `nu !` |
| 397 | nubo | `s l` |
| 400 | obelisko | `kaz` |
| 400 | obeza | `lia` |
| 401 | obskurantismo | `dei` |
| 404 | okaziono | `oka` |
| 404 | oktanto | `to-` |
| 404 | okupar | `Pos` |
| 407 | opozar | `tru` |
| 408 | oraklo | `Dic` |
| 409 | ordinar | `e -` |
| 410 | oriento | `rep` |
| 411 | ortopedio | `a k` |
| 414 | oxalo | `zas` |
| 415 | paf! | `paf !` |
| 415 | pajo | `rti` |
| 415 | pako | `DEF` |
| 416 | palaco | `ejo` |
| 416 | palankino | `sua` |
| 418 | pano | `k m` |
| 420 | paplo | `e r` |
| 421 | parafo | `sua` |
| 421 | parafo | `uan` |
| 422 | pardonar | `ulo ad ulu - o : ulu pri ulo` |
| 422 | parentezo | `- I` |
| 422 | parento | `mem` |
| 423 | parlamento | `i", q` |
| 423 | partenero | `nti` |
| 423 | parto | `mal` |
| 424 | pasabla | `tol` |
| 424 | pasajar | `s l` |
| 426 | patento | `eto` |
| 427 | patologio | `gio` |
| 427 | patrimonio | `qua` |
| 429 | pedonklo | `klo` |
| 429 | pektar | `o q` |
| 430 | penacho | `kaz` |
| 432 | perigeo | `per` |
| 433 | peritoneo | `ajo` |
| 433 | perkutar | `arm` |
| 434 | perono | `nas` |
| 435 | pesimismo | `ala` |
| 436 | petro | `rsa` |
| 436 | petroselo | `num` |
| 438 | pilo | `rti` |
| 439 | pilono | `pil` |
| 440 | pintado | `qua` |
| 440 | piro | `qua` |
| 441 | piruetar | `DEF` |
| 442 | pitono | `nto` |
| 443 | plano | `uro` |
| 444 | plankton | `i o` |
| 444 | plantago | `tip` |
| 444 | plasmodio | `pla` |
| 445 | plastrono | `o d` |
| 446 | plendar | `tiv` |
| 448 | plumo | `nta` |
| 448 | plump! | `plump !` |
| 448 | plus | `kun "ne" avan` |
| 448 | plusho | `a v` |
| 449 | polexo | `o m` |
| 452 | ponderar | `a v` |
| 453 | pordo | `Kla` |
| 453 | pornografo | `zas` |
| 454 | portfolio | `ita` |
| 457 | *prei | `+ prei` |
| 457 | *prei | `+pliz` |
| 461 | prizentar | `a k` |
| 462 | profanacar | `car` |
| 465 | propoziciono | `eol` |
| 466 | prostezo | `ica` |
| 468 | protokolo | `olo` |
| 470 | psit! | `psit !` |
| 470 | pubio | `ral` |
| 472 | pupeo | `kar` |
| 472 | pupeo | `uza` |
| 473 | pupitro | `qua` |
| 475 | quak! | `quak !` |
| 481 | rakar | `kar` |
| 481 | ralo | `alo` |
| 481 | ramno | `e d` |
| 481 | ranca | `nca` |
| 482 | rango | `aso` |
| 483 | ravelino | `rav` |
| 484 | reaktar | `mio` |
| 485 | redaktar | `bro` |
| 486 | referendario | `rda` |
| 488 | rehabilitar | `a d` |
| 488 | reino | `ido` |
| 488 | reklamacar | `lar` |
| 488 | rekomendar | `afr` |
| 489 | rektoro | `niv` |
| 490 | religio | `tik` |
| 491 | remonto | `esa` |
| 491 | remonto | `mil` |
| 491 | remulado | `kun` |
| 491 | rentiro | `dus` |
| 494 | requiem | `Meso di "requiem` |
| 494 | respondar | `ues` |
| 497 | rezervar | `uzesas nur ye la formo "esar rezervita` |
| 497 | rezino | `uas` |
| 498 | rezumar | `plu` |
| 499 | rimo | `ala` |
| 501 | rokambolo | `m k` |
| 501 | roko | `a r` |
| 501 | roko | `par` |
| 502 | romantika | `gar` |
| 503 | rotifero | `ete` |
| 504 | ruda | `ne-` |
| 508 | sakra | `a a` |
| 508 | sakrifikar | `sak` |
| 508 | sakristo | `aro` |
| 510 | samovaro | `qua` |
| 515 | seciono | `kta` |
| 517 | semolo | `o o` |
| 521 | *sesiono | `sea` |
| 521 | serpento | `ova` |
| 522 | sexuo | `III` |
| 523 | shablono | `qua` |
| 523 | shafto | `par` |
| 523 | shakalo | `aur` |
| 523 | shakar | `lda` |
| 523 | shakar | `qua` |
| 523 | shamar | `sua` |
| 524 | shampunar | `o p` |
| 525 | shikanar | `hav` |
| 525 | shovina | `a p` |
| 525 | shut! | `shut !` |
| 526 | signo | `man` |
| 527 | silepso | `omi` |
| 529 | simfonio | `de la yarcento l8-esma` |
| 530 | sincera | `kte` |
| 530 | sindikato | `cas` |
| 532 | siropo | `qua` |
| 532 | siropo | `qua` |
| 535 | skeptika | `ono` |
| 536 | skolio | `eom` |
| 537 | skopso | `elo` |
| 537 | skreno | `+skrino` |
| 537 | skreno | `pro` |
| 538 | skrubo | `ixi` |
| 538 | slamo | `nas` |
| 538 | slamo | `sur` |
| 538 | sluzo | `DER` |
| 539 | smalto | `lua` |
| 539 | snapar | `axi` |
| 539 | sniflar | `Asp` |
| 539 | snortar | `rui` |
| 539 | snortar | `tar` |
| 539 | sodo | `ant` |
| 539 | sodo | `kti` |
| 539 | sofisto | `dar` |
| 539 | sofisto | `nst` |
| 542 | sorbo | `sor` |
| 544 | spektro | `tro` |
| 549 | st! | `st !` |
| 549 | stacar | `sur` |
| 550 | standardo | `tan` |
| 551 | stelo | `met` |
| 552 | stentoro | `ers` |
| 552 | stereoskopo | `u r` |
| 553 | stilo | `lof` |
| 556 | strio | `o g` |
| 557 | stulo | `-ap` |
| 557 | stuporar | `rpo` |
| 558 | stuvo | `alt` |
| 560 | sudoro | `ktu` |
| 562 | superstico | `kra` |
| 564 | sus! | `sus !` |
| 565 | *swobo | `+swobo` |
| 573 | tempo | `tur` |
| 573 | tempo | `ubi` |
| 574 | tempestar | `e c` |
| 575 | tenua | `ske` |
| 576 | teraso | `upr` |
| 581 | timuso | `lar` |
| 581 | tineo | `qua` |
| 581 | tineo | `qua` |
| 583 | tisuo | `nti` |
| 583 | titrar | `rea` |
| 590 | transitar | `ala` |
| 591 | traverso | `ukt` |
| 594 | triktrako | `obt` |
| 595 | tripo | `ner` |
| 597 | truismo | `tan` |
| 598 | trusto | `per` |
| 600 | tumultar | `tar` |
| 600 | turgecar | `e r` |
| 601 | tutelar | `ito` |
| 602 | *ultima | `a n` |
| 602 | ubiqua | `Qua` |
| 602 | uf! | `uf !` |
| 602 | ulno | `a m` |
| 603 | unanima | `Qua` |
| 603 | uniforma | `esa` |
| 604 | upupo | `pla` |
| 604 | uragano | `gen` |
| 606 | uzar | `kto` |
| 606 | uzukaptar | `per` |
| 607 | vacilar | `lar` |
| 608 | valerianelo | `anj` |
| 608 | valvo | `mpo` |
| 608 | vana | `nul` |
| 610 | ve! | `ve !` |
| 614 | vertijar | `las` |
| 615 | viatiko | `a p` |
| 615 | vibriono | `opoze a "bakterio` |
| 617 | vinagro | `ras` |
| 620 | volar | `III` |
| 622 | vulnerario | `edi` |
| 622 | vulto | `emb` |
| 623 | *werar | `+werar` |
| 623 | *wishar | `+wishar` |
| 629 | cerebelo | `bot` |

## Mots-outils seuls — 2

| page | mot-vedette | fragment souligne |
|---:|---|---|
| 523 | shagrino | `quan` |
| 523 | shalmo | `quan` |

## Coupe au milieu d'un mot, ou reste du mot-vedette — 1189

| page | mot-vedette | fragment souligne |
|---:|---|---|
| 1 | abasar | `er s` |
| 2 | abdikar | `ritato` |
| 3 | abolisar | `leji, sklaveso-rejimo` |
| 4 | absolvar | `u exp` |
| 4 | abutmento | `a su` |
| 5 | aceptar | `- II` |
| 6 | acizo | `osto` |
| 6 | adepto | `adepto.- Pe` |
| 7 | admirar | `trans, pri, pro` |
| 7 | adoptar | `metafor` |
| 8 | aerometro | `u po` |
| 8 | aeroskopo | `la p` |
| 9 | afusto | `artilrio` |
| 10 | agento | `agento. L` |
| 10 | aglo | `eras` |
| 11 | agrimonio | `agrimonia` |
| 11 | agrimonio | `eupatoria` |
| 11 | agro | `ular` |
| 12 | -ajo | `-(ant)ajo` |
| 12 | -ajo | `a(ta)jo` |
| 12 | akademio | `as, ge` |
| 12 | akanto | `tile` |
| 13 | akompanar | `uan k` |
| 13 | akordeono | `akord` |
| 14 | akreditar | `agnos` |
| 14 | akrostiko | `lite` |
| 14 | aktinio | `r baz` |
| 15 | akuta | `a. -` |
| 16 | albina | `la a` |
| 16 | albo | `i, e` |
| 17 | alegorio | `la s` |
| 18 | alimentar | `o, vi` |
| 20 | alotropa | `trig` |
| 21 | alternativo | `lternativo` |
| 21 | amaso | `as ma` |
| 22 | ambajo | `ambajo. S` |
| 23 | amfigenezo | `u, p` |
| 23 | amfioxo | `amphioxus lanceolatus.-` |
| 24 | amiko | `ua d` |
| 24 | amnioto | `s la` |
| 25 | -an- | `-an -` |
| 25 | anado | `anas` |
| 25 | anado | `anatis` |
| 26 | anafazo | `o mi` |
| 28 | angelus | `ye d` |
| 28 | angorar | `a su` |
| 29 | anhelar | `netrans, pro` |
| 30 | anterozoido | `anterozoid` |
| 31 | antinomio | `real` |
| 33 | aparta | `rita` |
| 33 | apartenar | `apartena` |
| 33 | apertar | `trans, por` |
| 33 | apie | `apie, (botaniko` |
| 34 | apostata | `is su` |
| 35 | apostrofo | `s qu` |
| 36 | apud | `ikas` |
| 38 | aristo | `i. -` |
| 39 | arkado | `apog` |
| 40 | arkoplasmo | `arkoplasmo.-(biol` |
| 42 | -as | `- as` |
| 42 | artezo-puteo | `teo b` |
| 42 | artichoko | `cynara` |
| 42 | artichoko | `scolymus` |
| 43 | asentar | `asepta` |
| 43 | asentar | `asesoro` |
| 43 | asepta | `asentar` |
| 43 | asepta | `asento` |
| 43 | asepta | `asesoro` |
| 43 | asepta | `konsento` |
| 43 | asepta | `trans` |
| 43 | asesoro | `asentar` |
| 43 | asesoro | `asento` |
| 43 | asesoro | `asepta` |
| 43 | asesoro | `konsento` |
| 43 | asesoro | `trans` |
| 45 | aspirar | `trans, per, ad` |
| 45 | asterismo | `ptik` |
| 46 | -at- | `- at -` |
| 46 | astriktar | `astr` |
| 46 | astriktar | `ktar` |
| 47 | -atr- | `- atr -` |
| 49 | auto- | `auto -` |
| 49 | autoklavo | `nter` |
| 49 | automata | `automata. Q` |
| 50 | autopsiar | `psiar` |
| 50 | avalanchar | `avalancha` |
| 50 | avalo | `ibar` |
| 53 | bakar | `trans, en` |
| 54 | balayar | `trans, per` |
| 57 | barikado | `etur` |
| 58 | barografo | `di l` |
| 58 | barsho | `labrax` |
| 58 | barsho | `lupus` |
| 59 | baskular | `baskula` |
| 59 | bataliar | `ataliar` |
| 59 | bataliono | `ataliono` |
| 59 | batar | `atar` |
| 61 | benedikar | `enedikar` |
| 62 | benefico | `bene` |
| 63 | betelo | `bete` |
| 63 | betonio | `betonica` |
| 63 | betonio | `officinalis` |
| 63 | bezigo | `o, d` |
| 63 | bezonar | `ezonar` |
| 63 | bi- | `iloba` |
| 63 | biblo | `iblo` |
| 64 | bideto | `sidila` |
| 64 | bifsteko | `cho d` |
| 66 | bistro | `koqu` |
| 67 | bivakar | `bivak` |
| 67 | bizono | `americanus` |
| 67 | blazono | `u rep` |
| 68 | bo- | `bo -` |
| 68 | boikotar | `boik` |
| 68 | boikotar | `per r` |
| 70 | bordo | `bordo. Part` |
| 70 | borgezo | `a ap` |
| 70 | boro | `borc` |
| 71 | bracho | `l su` |
| 71 | brakio | `no. -` |
| 72 | brancho | `ivers` |
| 72 | brankio | `a ae` |
| 74 | brizo | `brizo. Ve` |
| 75 | bubalo | `a, ku` |
| 76 | bugrano | `to d` |
| 76 | bujio | `no u` |
| 76 | buklo | `fixig` |
| 76 | bulbo | `orma` |
| 79 | butro | `la, f` |
| 79 | buzo | `buteo` |
| 79 | buzo | `falco` |
| 81 | celerio | `apium g` |
| 81 | celerio | `aveolens` |
| 82 | celuloido | `celluloido` |
| 82 | cementacar | `cementaca` |
| 82 | cemento | `ki pi` |
| 82 | cenobito | `cenobit` |
| 83 | centaureo | `chlora perfoliata ce` |
| 83 | centaureo | `taurea` |
| 84 | ceptro | `mpero-` |
| 84 | ceriz-lauro | `cerasus (prunus) laurocerasus` |
| 85 | ceruzo | `to e` |
| 85 | chamado | `a en` |
| 88 | chimo | `s, e` |
| 89 | cibolo | `allium fistu-` |
| 89 | cibolo | `losum` |
| 89 | cienco | `de f` |
| 90 | ciklido | `o di l` |
| 90 | ciklopo | `entat` |
| 91 | cilico | `religio. katolika` |
| 91 | cilindroido | `cilindroid` |
| 91 | cinamo | `cinam` |
| 91 | cinematiko | `konce` |
| 92 | cinika | `a su` |
| 94 | civeto | `martr` |
| 94 | cizelar | `un e` |
| 95 | dago | `is p` |
| 95 | dalmatiko | `an met` |
| 98 | defekto | `en p` |
| 98 | defiar | `defia` |
| 100 | dekretar | `aludante sutoritatozo suprega` |
| 102 | demonstrar | `ontrar` |
| 105 | derogar | `deroga` |
| 105 | desegnar | `trans, per, sur, e c` |
| 105 | desertar | `trans, e netrans` |
| 105 | destinar | `estinar` |
| 106 | detektoro | `ktoro` |
| 106 | determinar | `di p` |
| 106 | devastar | `i, c` |
| 106 | developar | `developa` |
| 107 | devota | `devot` |
| 107 | dextrino | `u uza` |
| 107 | diado | `iado` |
| 107 | dio | `o di` |
| 108 | diafragmo | `tiko` |
| 108 | diakoniso | `ismo` |
| 109 | dianto | `anto` |
| 109 | dicionario | `icionario` |
| 109 | diciono | `iciono` |
| 109 | diciplinar | `ioiplinar` |
| 110 | diferenciar | `a tis` |
| 112 | dilatar | `orpo` |
| 112 | dilutar | `meopatio` |
| 113 | dindo | `meleagris gallo-` |
| 113 | dindo | `pavo` |
| 114 | diplomatiko | `veri` |
| 114 | direta | `diret` |
| 115 | disko | `korp` |
| 115 | diskreta | `diskr` |
| 115 | diskriminanto | `diskrim` |
| 115 | diskriminanto | `nanto` |
| 116 | diskutar | `nte personi qui ne havas` |
| 116 | dispensar | `disp` |
| 116 | dispensar | `nsar` |
| 116 | dispensario | `disp` |
| 117 | ditreso | `ditreso.-` |
| 118 | divergar | `aludante eleme` |
| 118 | divergar | `ti plu o min inter-apuda` |
| 118 | diversa | `aludante kozi quin onu kompar` |
| 118 | diversa | `diver` |
| 118 | diversionar | `ersionar` |
| 118 | divinar | `er k` |
| 118 | diviziono | `s ek` |
| 119 | dogmato | `dogmato. Kr` |
| 119 | dolio | `olio` |
| 120 | dolmano | `exem` |
| 120 | dolmeno | `a, po` |
| 120 | dolomito | `-mar` |
| 120 | domo | `ktur` |
| 121 | dominanto | `kristalografi` |
| 121 | donacar | `nar a` |
| 121 | dorado | `ryphaena hippurus` |
| 122 | drenar | `guli` |
| 123 | driftar | `riftar` |
| 123 | dronar | `trand` |
| 123 | dronto | `ineptus` |
| 123 | duelar | `a altra la honor-satisfaco (pos ofenseso) per armo-` |
| 123 | duelar | `ukto` |
| 124 | duko | `eren` |
| 125 | dushar | `r (la` |
| 127 | ecitar | `anis` |
| 127 | edero | `e qu` |
| 129 | eglefino | `gadus` |
| 129 | eglefino | `glefino` |
| 129 | egorjar | `gorjar` |
| 129 | egotismo | `gotismo` |
| 129 | ekidno | `echidna (hystrix` |
| 130 | el(u) | `el(u` |
| 131 | elefanto | `lua ma` |
| 131 | elektro | `ukta` |
| 132 | elevatoro | `niko` |
| 133 | embarasar | `erpl` |
| 135 | empireo | `la te` |
| 136 | emula | `esar` |
| 136 | endemika | `a es` |
| 137 | engajar | `ngajar` |
| 137 | enkilemo | `nkilemo` |
| 137 | enklitiko | `nklitiko` |
| 137 | enluminar | `nluminar` |
| 138 | entamar | `pri kozo tillore netushita` |
| 138 | entimemo | `tacit` |
| 139 | entrenar | `entr` |
| 140 | eparko | `di Ko` |
| 140 | epidemio | `aspe` |
| 140 | epidermo | `i la p` |
| 142 | epodo | `iamb` |
| 142 | epoko | `to qua` |
| 142 | equatoro | `ko d` |
| 142 | equipar | `o, a` |
| 142 | equipolar | `equip` |
| 143 | eritrocito | `ritrocito` |
| 145 | eskadro | `eskadro. (Navaro` |
| 145 | eskamotar | `otar` |
| 145 | eskaro | `en p` |
| 145 | eskopeto | `skopeto` |
| 147 | esperar | `sperar` |
| 148 | estragono | `artemisia dracuncul` |
| 148 | esturjono | `a ov` |
| 148 | esturjono | `esturjon` |
| 149 | etero | `kemi` |
| 149 | etiko | `etiko. I` |
| 149 | etuyo | `xo a` |
| 150 | eunuko | `la mul` |
| 151 | exajerar | `xajerar` |
| 151 | exaltar | `velo` |
| 151 | exekutar | `o kon` |
| 152 | exemplero | `o en` |
| 154 | expiacar | `desp` |
| 154 | expirar | `pri vejetant` |
| 154 | explorar | `stud` |
| 155 | extorsar | `xtorsar` |
| 156 | -ez | `-ez-` |
| 156 | extraktar | `a. - V` |
| 156 | extrema | `ye g` |
| 156 | extrinseka | `extrinsek` |
| 157 | fabo | `faba` |
| 157 | fabo | `vicia` |
| 157 | fabrikar | `fabrika` |
| 157 | facila | `acila` |
| 157 | facionar | `acionar` |
| 157 | fagocito | `agocito` |
| 157 | fagocitoso | `agocitoso` |
| 157 | fairo | `airo` |
| 158 | fakturo | `fakt` |
| 159 | falchar | `alchar` |
| 159 | falco | `alco` |
| 159 | faldar | `aldar` |
| 159 | faleno | `aleno` |
| 159 | familio | `icina` |
| 159 | fanatika | `iras` |
| 160 | fantasmagorio | `qui l` |
| 161 | farmar | `farm` |
| 162 | feo | `nu re` |
| 163 | femina | `o or` |
| 163 | fenikulo | `fenikulc` |
| 165 | fidar | `idar` |
| 166 | filatelio | `imbri` |
| 168 | finko | `fringilla` |
| 169 | flakono | `o, k` |
| 169 | flanelo | `lanelo` |
| 169 | flanjo | `lanjo` |
| 171 | fluktuar | `luktuar` |
| 173 | fomentar | `fomenta` |
| 174 | forcato | `forc` |
| 175 | formulo | `dete` |
| 175 | formulo | `ormulo` |
| 175 | forta | `alud` |
| 175 | forta | `ludante la anmo` |
| 175 | forta | `te korpo` |
| 176 | fotografar | `o (o` |
| 177 | foxo | `canis` |
| 177 | foxo | `vulpes` |
| 177 | framasono | `framaso` |
| 178 | francisko | `francisk` |
| 178 | frankolino | `frank` |
| 178 | frankolino | `lino` |
| 179 | fremisar | `isar` |
| 179 | frigoro | `rigoro` |
| 180 | frondo | `rondo` |
| 181 | frunsar | `frunsa` |
| 182 | fuino | `foina` |
| 182 | fuino | `mustela` |
| 182 | fulko | `gallinula c` |
| 182 | fulko | `loropus` |
| 183 | fundamento | `ua s` |
| 184 | furnazo | `furna` |
| 184 | fusto | `artilrio` |
| 184 | fuxino | `fuxi` |
| 187 | gangliono | `angliono` |
| 187 | gangrenar | `angrenar` |
| 187 | ganso | `anso` |
| 192 | germanio | `germ` |
| 193 | giganto | `giganto. P` |
| 194 | gitago | `agrostemma` |
| 194 | gitago | `githago` |
| 195 | glicino | `licino` |
| 195 | glikogena | `likogena` |
| 196 | glosario | `glosar` |
| 197 | gluglar | `guglar` |
| 197 | gnuo | `catoblepas` |
| 198 | gotika | `a ye` |
| 199 | grafismo | `grafism` |
| 199 | granario | `ranario` |
| 199 | granato | `ranato` |
| 201 | grilar | `koqua` |
| 202 | griliotalpo | `gryllotalpa` |
| 202 | griliotalpo | `gryllus` |
| 202 | griliotalpo | `iotalpo` |
| 202 | grincar | `incar` |
| 202 | grondar | `gronda` |
| 202 | grosiera | `pri person` |
| 203 | guardo | `a la r` |
| 203 | guardo | `uardo` |
| 204 | guindo | `juliana` |
| 205 | guvernar | `guvern` |
| 205 | guyavo | `guyaba` |
| 205 | guyavo | `psidium` |
| 207 | hancho | `ro di` |
| 207 | handikapar | `andikapar` |
| 207 | hangaro | `angaro` |
| 207 | hanso | `anso` |
| 207 | haptiko | `aptiko` |
| 208 | harda | `aludant` |
| 209 | hederaceo | `hederace` |
| 211 | helpar | `lpar` |
| 211 | hemoglobino | `emoglobino` |
| 211 | hemolizo | `emolizo` |
| 211 | hemoptizio | `emoptizio` |
| 211 | hemoragio | `emoragio` |
| 211 | hemoroido | `emoroido` |
| 212 | heraldo | `cheval` |
| 212 | heraldo | `erala` |
| 212 | herisar | `udante homo od animalo` |
| 214 | hibernar | `hiber` |
| 215 | hidramnioto | `hidramniot` |
| 215 | hidrauliko | `auliko` |
| 216 | hidrometro | `hidrometr` |
| 217 | hiperbato | `iperbato` |
| 220 | historiografo | `historiogra` |
| 221 | hola! | `hola` |
| 221 | holoturio | `thuria` |
| 221 | homardo | `homarus` |
| 221 | homardo | `ulgaris` |
| 222 | honoro | `da n` |
| 224 | humuso | `as e` |
| 224 | hundo | `canis` |
| 224 | hundo | `familiaris` |
| 225 | husaro | `a) k` |
| 227 | ikonoklasto | `konoklasto` |
| 228 | imaginara | `aginara` |
| 229 | imajo | `vide` |
| 229 | imanar | `imana` |
| 229 | imbastar | `imbasta` |
| 229 | imbecila | `imbecil` |
| 229 | imbrikar | `imbrika` |
| 229 | imensa | `imens` |
| 229 | imersar | `imersa` |
| 229 | imitar | `imita` |
| 229 | imortelo | `imortel` |
| 229 | impedar | `impeda` |
| 230 | imperfekto | `imperfekt` |
| 230 | implicita | `implicit` |
| 230 | implorar | `impl` |
| 230 | impregnar | `impregnan` |
| 230 | impresar | `impresa` |
| 231 | impresiono | `pikt, e literat` |
| 231 | impresiono | `presiono` |
| 231 | incendiar | `incendia` |
| 231 | incenso | `ncenso` |
| 231 | incestar | `ncestar` |
| 231 | incidar | `ncida` |
| 231 | incidento | `ncidento` |
| 231 | incitar | `ncitar` |
| 231 | incizar | `ncizar` |
| 232 | -ind- | `-ind` |
| 233 | infantrio | `infa` |
| 234 | inferno | `infern` |
| 235 | informar | `informa` |
| 235 | infre | `priobjekto vertikala` |
| 235 | infuzar | `infuza` |
| 235 | infuzorio | `infuzori` |
| 235 | ingestar | `ingesta` |
| 235 | inglobar | `ingloba` |
| 235 | ingranar | `ingrana` |
| 235 | ingrediento | `a ko` |
| 235 | ingrediento | `ingredient` |
| 235 | inguino | `inguin` |
| 235 | inhalar | `inhala` |
| 235 | inherar | `inhera` |
| 235 | inhibar | `inhiba` |
| 235 | iniciar | `inicia` |
| 235 | iniciativar | `iniciativar(trans` |
| 235 | injektar | `injekta` |
| 235 | injeniar | `injenia` |
| 235 | injenioro | `injenior` |
| 237 | inquestar | `nquestar` |
| 237 | inquiziciono | `nquiziciono` |
| 237 | inquizitoro | `inqui` |
| 237 | inquizitoro | `itoro` |
| 237 | insekto | `nsekto` |
| 237 | insertar | `nsertar` |
| 237 | insinuar | `etaf` |
| 237 | insipida | `a rep` |
| 237 | insolenta | `insolent` |
| 238 | insulo | `n mar` |
| 239 | intensa | `ntensa` |
| 239 | inter- | `nter-` |
| 239 | interceptar | `nterceptar` |
| 239 | intercesar | `ntercesar` |
| 239 | interdiktar | `nterdiktar` |
| 239 | interesar | `nteresar` |
| 239 | interesto | `nteresto` |
| 239 | interferar | `nterferar` |
| 239 | interimo | `nterimo` |
| 239 | intermezo | `ntermezo` |
| 239 | intermitar | `ntermitar` |
| 239 | interna | `nterna` |
| 239 | internacionalo | `nternacionalo` |
| 239 | interogativo | `nterogativo` |
| 240 | interviuvar | `aludante la mrnalisti` |
| 240 | intestino | `kozi` |
| 241 | invalida | `aludante ex-` |
| 241 | invalida | `militisto` |
| 242 | inventariar | `trans.) Ko` |
| 242 | inyamo | `acei` |
| 242 | ionika | `aludante la triesma ek la kin klasA` |
| 243 | irigacar | `irigaca` |
| 243 | iruptar | `ruptar` |
| 243 | iskiono | `skiono` |
| 245 | izolar | `lu e` |
| 245 | izometra | `izometr` |
| 246 | jalapo | `i qua` |
| 246 | janicharo | `sold` |
| 247 | jelozio | `kita` |
| 248 | jermo | `- II` |
| 248 | jilfloro | `cheiranthus cheir` |
| 248 | jineto | `iverra` |
| 250 | junio | `ato s` |
| 250 | juntar | `aludante plura personi, trupi,e c` |
| 251 | ka(d) | `ka(d` |
| 251 | kabrar | `abrar` |
| 251 | kabrioleto | `abrioleto` |
| 251 | kachuo | `achuo` |
| 251 | kadastro | `adastro` |
| 252 | kaduceo | `ti p` |
| 252 | kafeo | `- II` |
| 253 | kalamito | `styrax calamit` |
| 254 | kalenturo | `ye c` |
| 255 | kalkaneo | `kalkane` |
| 255 | kalsono | `alsono` |
| 255 | kalumniar | `alumniar` |
| 255 | kalva | `alva` |
| 256 | kameo | `a st` |
| 256 | kameo | `kame` |
| 257 | kamiono | `amiono` |
| 257 | kampaniar | `ampaniar` |
| 257 | kampanulo | `ampanulo` |
| 257 | kampar | `ampar` |
| 257 | kampar | `o de` |
| 258 | kampesho | `kampesh` |
| 258 | kanabino | `cannabina` |
| 258 | kanabino | `fringilla` |
| 258 | kanalio | `kanali` |
| 258 | kandida | `e lua` |
| 258 | kano | `arund` |
| 258 | kano | `phragmites` |
| 259 | kanina | `he l` |
| 259 | kankro | `zool.)I` |
| 259 | kanoniko | `anoniko` |
| 259 | kanoo | `anoo` |
| 260 | kantarido | `e en` |
| 261 | kaoso | `mataf` |
| 261 | kapero | `capparia spi` |
| 261 | kapero | `nosa` |
| 262 | kapono | `a qu` |
| 262 | kaprimulgo | `caprimalgus` |
| 262 | kaprimulgo | `europeus` |
| 262 | kapro | `capra (hircus` |
| 263 | kapsulo | `artilri` |
| 263 | kapuchino | `apuchino` |
| 263 | kapucino | `apucino.(kristanismo` |
| 263 | karabino | `arabino` |
| 263 | karabino | `esas` |
| 263 | karabo | `arabo` |
| 263 | karafo | `gene` |
| 263 | karakolar | `istre` |
| 264 | karasino | `carassius` |
| 264 | karasino | `vulgaris` |
| 264 | karavano | `zerti` |
| 264 | karbonario | `carbonarius` |
| 264 | karbonario | `gadus` |
| 265 | kardar | `e kri` |
| 265 | kareno | `areno` |
| 267 | karpentar | `zesa` |
| 267 | kartamino | `artamino` |
| 267 | kartamo | `car-` |
| 267 | kartamo | `thamus tinctorius` |
| 269 | kaskado | `ntometaf` |
| 270 | kastanyeto | `or i` |
| 270 | katafalko | `katafalko. E` |
| 271 | katedralo | `atedralo` |
| 271 | katedro | `atedro` |
| 271 | kategorio | `ategorio` |
| 272 | katodo | `e mun` |
| 273 | kaviaro | `esora` |
| 274 | kazako | `kun manik` |
| 275 | kemio | `a el` |
| 275 | kerubo | `kerub` |
| 276 | kilo- | `kilo` |
| 276 | kilolitro | `kilolitr` |
| 276 | kimero | `magi` |
| 277 | kirsho | `irsho` |
| 279 | kliento | `omerco` |
| 280 | klishar | `fctogr` |
| 282 | koadjutoro | `ioni` |
| 282 | kobeo | `cobea (scanden` |
| 282 | kocinelo | `coccinela (septempunctata` |
| 282 | kodexo | `i far` |
| 283 | koercitiva | `oercitiva` |
| 283 | kokardo | `okardo` |
| 283 | kokleo | `okleo` |
| 284 | kolchiko | `autumnale` |
| 284 | kolchiko | `colchicum` |
| 284 | kolegio | `cipal` |
| 285 | koleoptero | `a eli` |
| 286 | kolonio | `lita f` |
| 286 | koloquinto | `colycynthis` |
| 286 | koloquinto | `cucumis` |
| 287 | koluteo | `arborescens` |
| 287 | koluteo | `colutea` |
| 287 | koluzionar | `ciisto` |
| 287 | koluzionar | `sekr` |
| 287 | kolzo | `brassica napus` |
| 287 | kolzo | `oleifera` |
| 287 | komandar | `andar` |
| 287 | komanditar | `omanditar` |
| 287 | komao | `omao` |
| 288 | kombinatoriko | `kombinatorik` |
| 288 | kombustar | `kombusta` |
| 288 | komencar | `komenca` |
| 289 | komisar | `omisar` |
| 289 | komisario | `omisario` |
| 289 | komisiono | `mand` |
| 289 | komisiono | `omisiono` |
| 289 | komisuro | `omisuro` |
| 289 | komitato | `omitato` |
| 289 | komocionar | `omocionar` |
| 289 | komodo | `modo` |
| 289 | komodoro | `niana ed Usana` |
| 289 | komodoro | `omodoro` |
| 289 | kompano | `ompano` |
| 290 | kompleta | `s nu` |
| 291 | komplikar | `trans, per` |
| 291 | kompunciono | `teol. kato` |
| 291 | komunikar | `omunikar` |
| 292 | koncienco | `erna` |
| 293 | kondamnar | `i libri` |
| 293 | kondoro | `gryphus` |
| 293 | kondoro | `sarcorhamphus` |
| 294 | konfidencar | `trans, ad` |
| 296 | konjurar | `rak-` |
| 297 | konkava | `onkava` |
| 297 | konklavo | `onklavo` |
| 297 | konkluzar | `onkluzar` |
| 297 | konko | `onko` |
| 297 | konkoido | `onkoido` |
| 297 | konkurencar | `aluda` |
| 297 | konkurencar | `te personi qui persequas la sama` |
| 298 | konoido | `olido` |
| 298 | konsentar | `arte` |
| 298 | konsequar | `konse` |
| 299 | konskriptar | `unto` |
| 299 | konsolidar | `aludan` |
| 299 | konsolidar | `e la debajo publik` |
| 299 | konsonancar | `aludante silabi, e pre` |
| 299 | konsonanto | `onsonanto` |
| 299 | konsorto | `onsorto` |
| 301 | kontinento | `e sen` |
| 301 | kontingento | `a kontin` |
| 301 | kontrabaso | `ntrabaso` |
| 302 | kontradanso | `ta s` |
| 302 | kontroversar | `kontroversar. (trans` |
| 303 | konvento | `mblit` |
| 305 | korano | `a en` |
| 305 | kordio | `la ka` |
| 306 | korinto | `e ni` |
| 307 | kormorano | `phalacrocorax (carbo` |
| 308 | kornico | `ariktekt` |
| 308 | korporalo | `ro, l` |
| 308 | korpulenta | `esas` |
| 308 | korpuskulo | `orpuskulo` |
| 308 | korsajo | `korsaj` |
| 309 | korsaro | `orsaro` |
| 309 | korseto | `orseto` |
| 309 | korto | `yuro-` |
| 309 | korveto | `navig.) Mil` |
| 310 | kosmografio | `la s` |
| 310 | kostumo | `opra` |
| 310 | kotiledono | `io e` |
| 310 | kotiliono | `ua fi` |
| 311 | krabo | `cancer` |
| 311 | krabo | `carcinus` |
| 311 | krapular | `ar g` |
| 312 | kratero | `ertu` |
| 312 | krear | `aludant` |
| 312 | kredar | `senco ab-` |
| 312 | kredar | `soluta` |
| 312 | kremo | `nu var` |
| 313 | kreso | `river` |
| 314 | kretino | `etino` |
| 314 | kreto | `yono` |
| 314 | kreveto | `crangon vul-` |
| 314 | kreveto | `garis` |
| 315 | kriptogama | `riptogama` |
| 316 | krizantemo | `i, f` |
| 318 | krucifera | `rucifera` |
| 318 | kruda | `aludante materio quan onu sub-` |
| 318 | kruda | `misas a preparo` |
| 320 | kukulo | `kulo` |
| 322 | kupleto | `kans` |
| 322 | kurar | `kura` |
| 323 | kuratar | `yurosienco` |
| 323 | kurkumo | `nteyi` |
| 324 | kustumar | `a ma` |
| 325 | kutio | `utio` |
| 325 | kuvento | `kuvent` |
| 326 | la | `determinata d` |
| 327 | lakto | `iero` |
| 327 | laktometro | `aktometro` |
| 327 | laktoso | `aktoso` |
| 327 | lakuno | `akuno` |
| 328 | lamarckismo | `ve la` |
| 329 | lantano | `antanakemio` |
| 331 | latiro | `l a t i r o` |
| 331 | laubo | `aubo` |
| 331 | laudar | `audar` |
| 332 | laureato | `kron` |
| 333 | legendo | `do e` |
| 333 | lego | `norm` |
| 334 | lektoro | `r la` |
| 334 | lentisko | `lentiscus` |
| 334 | leono | `oneg` |
| 335 | leporo | `lepus` |
| 335 | leporo | `timidus` |
| 335 | letargio | `as i` |
| 335 | letro | `or k` |
| 335 | levriero | `levrier` |
| 336 | lezar | `uro-cienco` |
| 336 | liano | `apta` |
| 337 | lienterio | `lienteri` |
| 337 | lietnanto | `la gra` |
| 337 | lignino | `ignino` |
| 337 | lignito | `ignito` |
| 337 | liguo | `iguo` |
| 339 | linchar | `lincha` |
| 339 | linealo | `inealo` |
| 339 | lineamento | `ineamento` |
| 339 | lineara | `ineara` |
| 339 | lingoto | `ingoto` |
| 340 | linguo | `lingu` |
| 340 | liquida | `liquid` |
| 340 | liquidacar | `o e p` |
| 341 | litotomiar | `itotomiar` |
| 342 | liuto | `sube-` |
| 342 | lobo | `ua f` |
| 343 | logiko | `enco` |
| 343 | lokacar | `okacar` |
| 343 | lokativo | `okativo` |
| 343 | loklo | `oklo` |
| 343 | loko | `dete` |
| 343 | lokomobilo | `okomobilo` |
| 345 | loxodromo | `s kon` |
| 345 | luciolo | `italica` |
| 345 | luciolo | `luciola` |
| 346 | lunatika | `kulta` |
| 347 | luxacar | `uxacar` |
| 348 | madono | `o, q` |
| 348 | madras | `madra` |
| 348 | mago | `mago.I` |
| 349 | magnolio | `ele-` |
| 349 | magra | `agra` |
| 349 | magra | `alud` |
| 349 | magra | `aluda` |
| 349 | magra | `nte homo od animalo vivanta` |
| 349 | magra | `te karno koquala` |
| 349 | majora | `ri terco` |
| 350 | makiavelala | `te ko` |
| 350 | makio | `mio ku` |
| 350 | makrosporo | `makrosppro` |
| 351 | makulaturo | `o-fo` |
| 351 | mala | `ea a` |
| 351 | maledikar | `meledikar` |
| 351 | maleolo | `tera` |
| 351 | malio | `ek l` |
| 352 | malvarozo | `thaea rosea` |
| 352 | mandarino | `citrus nobi-` |
| 353 | mandolino | `e qui` |
| 353 | mandragoro | `trib` |
| 354 | maniplo | `liturgio kato` |
| 355 | manovrar | `alud` |
| 356 | maraskino | `atafi` |
| 358 | marono | `aesculus` |
| 358 | marono | `hippocastanum` |
| 359 | mashinacar | `mashinaca` |
| 359 | maskulo | `askulo` |
| 359 | masonar | `asonar` |
| 359 | masonar | `ponto, l o` |
| 360 | mastico | `duri` |
| 360 | matraso | `esas` |
| 361 | matrico | `ldur` |
| 361 | matrono | `o re` |
| 361 | mazurko | `am va` |
| 362 | mediata | `mediat` |
| 363 | meduzo | `ofito` |
| 363 | megaterio | `megater` |
| 364 | meliso | `la fa` |
| 365 | membrano | `embrano` |
| 365 | memorialo | `memorial` |
| 367 | merizo | `avium` |
| 367 | merizo | `cerasus` |
| 367 | mesajo | `i la` |
| 368 | metaloido | `ande` |
| 369 | metamorfosar | `etamorfosar` |
| 369 | metano | `etano` |
| 369 | metastabila | `etastabila` |
| 369 | meteorologio | `izik` |
| 371 | migreno | `rala` |
| 371 | mikao | `ikao` |
| 371 | mikologo | `ikologo` |
| 372 | mikra | `la h` |
| 372 | mikrografo | `mikrograf` |
| 372 | mikrono | `di m` |
| 373 | milyeto | `ilieto` |
| 373 | mimar | `imar` |
| 373 | mimoso | `imoso` |
| 375 | minucio | `talo` |
| 376 | mirmekofila | `ofila` |
| 376 | mirtelo | `mirte` |
| 376 | mirtelo | `murtidus` |
| 376 | mirtelo | `vaccinium` |
| 376 | mispelo | `mes-` |
| 376 | mispelo | `pilus` |
| 378 | modiliono | `o du` |
| 378 | modlar | `dante la materio quan onu fasonas` |
| 378 | modo | `yuro-cienc` |
| 383 | mortifikar | `ortifikar` |
| 384 | moskeo | `moskeo.(religio` |
| 385 | mucilajo | `farmaci` |
| 386 | multiplikar | `likar` |
| 386 | multiplikar | `ulti` |
| 386 | mumio | `mumi` |
| 386 | mureno | `mure` |
| 386 | murmurar | `murm` |
| 387 | mutono | `aries` |
| 387 | mutono | `ovis` |
| 388 | mutuala | `inte` |
| 388 | muzelo | `uzelo` |
| 388 | muzeo | `oki antiqua` |
| 389 | nadiro | `e qu` |
| 391 | nebuloso | `nebulos` |
| 392 | neologismo | `nove-` |
| 392 | nervaturo | `- Tub` |
| 392 | nervaturo | `orna` |
| 392 | nervaturo | `tekturo` |
| 392 | nervaturo | `uro s` |
| 394 | nicho | `ra r` |
| 396 | nomar | `o qu` |
| 398 | nukleino | `la ce` |
| 398 | numero | `ndikas` |
| 398 | numismatiko | `numismatiko. L` |
| 398 | numulario | `machia numularia` |
| 400 | o(d) | `o (d` |
| 400 | obliqua | `as d` |
| 401 | obolo | `di d` |
| 402 | oceloto | `felis` |
| 402 | oceloto | `pardalia` |
| 403 | -oid | `- oid` |
| 403 | oficiro | `as of` |
| 403 | ofikleido | `o, q` |
| 403 | ogivo | `i qui` |
| 403 | oglar | `glar` |
| 404 | okaziono | `iono` |
| 404 | okro | `iabl` |
| 404 | oktopodo | `octo-` |
| 404 | ol(u) | `ol(u` |
| 405 | on(u) | `on(u` |
| 408 | orangutano | `e la` |
| 409 | ordinar | `ordin` |
| 409 | organo | `gano` |
| 409 | organzino | `krud` |
| 411 | ortografika | `rtografika` |
| 411 | ortokrona | `rtokrona` |
| 411 | ortolano | `rtolano` |
| 411 | ortopedio | `rtopedio` |
| 413 | ovoida | `voida` |
| 414 | -oz- | `- oz-` |
| 415 | pagino | `la d` |
| 416 | paketboto | `s, t` |
| 417 | paliumo | `en la epoki antiqu` |
| 418 | panduro | `pandur` |
| 418 | panegiro | `panegir` |
| 418 | panelo | `maso` |
| 418 | pangeno | `ilive` |
| 419 | pankraco | `en la epoqui antiqua` |
| 419 | panoramo | `anke meta` |
| 419 | panteismo | `nosk` |
| 420 | papayo | `as e` |
| 420 | papayo | `carica` |
| 420 | papayo | `papaya` |
| 420 | papero | `part` |
| 420 | parabolo | `as d` |
| 421 | parafo | `s en` |
| 421 | paralaxo | `utas a` |
| 422 | parfumo | `agreab` |
| 422 | parieto | `c. -` |
| 422 | parieto | `pariet` |
| 424 | participo | `di l` |
| 426 | pateno | `por l` |
| 426 | patero | `za p` |
| 426 | patino | `onas` |
| 427 | patologio | `patol` |
| 427 | patrono | `en Roma, dum la epo` |
| 429 | pedikulo | `pato` |
| 429 | pedikuloso | `ikuloso` |
| 429 | pedonklo | `pedo` |
| 429 | pegazo | `itol` |
| 429 | pektoso | `un ku` |
| 430 | pelargonio | `nium` |
| 430 | pelargonio | `pelargo-` |
| 430 | pelerino | `peleri` |
| 430 | pelikulo | `pelikul` |
| 430 | pelikulo | `r la` |
| 431 | pensionar | `r loje` |
| 431 | penta- | `penta` |
| 431 | pentametro | `pentametr` |
| 432 | periferio | `o di f` |
| 433 | periskopo | `arna` |
| 433 | perkalino | `erkalino` |
| 433 | perkutar | `edicino` |
| 433 | perkutar | `erkutar` |
| 434 | peroraciono | `peroracion` |
| 434 | perpendikla | `perpendikl` |
| 434 | perplexa | `asat` |
| 434 | perquisitar | `itar` |
| 434 | perquisitar | `perqui` |
| 435 | perseverar | `pers` |
| 435 | perseverar | `verar` |
| 436 | petroselo | `petroseli-` |
| 437 | pichpino | `pinus palustris (australis` |
| 437 | piedestalo | `lo di` |
| 437 | piedestalo | `pied` |
| 437 | piedestalo | `stalo` |
| 437 | pigargo | `circus` |
| 437 | pigargo | `pugargus` |
| 439 | pimento | `bril` |
| 440 | pintado | `pintad` |
| 441 | piretro | `pyreth` |
| 441 | pirogo | `cita` |
| 441 | pironismo | `ofis` |
| 442 | pistar | `trans.)R` |
| 443 | plako | `maxi` |
| 443 | planeto | `astron. anti` |
| 443 | plano | `co plana` |
| 444 | plasmodio | `modio` |
| 444 | plastido | `stido` |
| 445 | plastidulo | `la e` |
| 446 | plesiosauro | `i st` |
| 446 | pleuronekto | `onekto` |
| 446 | pleuronekto | `pleu` |
| 447 | pleyado | `ala, Ido` |
| 447 | pleyado | `di la linguo univer` |
| 447 | plorar | `plorar(netrans` |
| 449 | poezio | `iero` |
| 449 | poli- | `poli` |
| 450 | poliglota | `tita` |
| 450 | polipo | `deve` |
| 450 | politeknika | `politeisto` |
| 451 | polstero | `u ne` |
| 452 | *pondar | `pompono` |
| 452 | pompono | `aludante ucelino` |
| 452 | pompono | `pondar` |
| 453 | porelo | `allium` |
| 453 | porelo | `porrum` |
| 454 | portreto | `pikt` |
| 454 | portuo | `ta p` |
| 455 | posturar | `aludante persono qua esas ava` |
| 459 | pretoro | `irek` |
| 460 | primadono | `ntist` |
| 460 | primulo | `mula officinalis` |
| 460 | primulo | `pri-` |
| 461 | prismato | `u, re` |
| 461 | privilejo | `indi` |
| 461 | probabla | `probab` |
| 462 | profanacar | `profan` |
| 462 | profesiono | `rofesiono` |
| 462 | profesoro | `fesoro` |
| 462 | profeto | `ofeto` |
| 463 | prognozar | `prognoza` |
| 463 | projektar | `skrino,p` |
| 465 | propozar | `propoza` |
| 466 | propulsar | `trans, ad` |
| 466 | prorogar | `ogar` |
| 467 | protestar | `aludante pago-mromiso, trato` |
| 468 | protokolo | `proto` |
| 469 | pseudo- | `pseudo` |
| 471 | pudlar | `ar l` |
| 471 | pudorar | `netran` |
| 471 | pudro | `reme` |
| 471 | pulo | `Lor ludo per karti, biliardo, en la parii pri kavalkuri` |
| 473 | purpuro | `murex brancari` |
| 474 | quadranto | `quadrant` |
| 474 | quadri- | `quadri` |
| 475 | quago | `hippotigris` |
| 475 | quago | `quagga` |
| 476 | quaza | `leso` |
| 479 | rabatar | `rabata` |
| 480 | radiometro | `to u` |
| 481 | raguto | `guto` |
| 481 | raitro | `itro` |
| 481 | rakahuto | `kahuto` |
| 481 | raketo | `aketo` |
| 481 | rakito | `akito` |
| 481 | rakontar | `akontar` |
| 481 | raliar | `aliar` |
| 481 | ramio | `amio` |
| 481 | ramo | `genea` |
| 481 | rampo | `ampo` |
| 482 | raptar | `aptar` |
| 483 | rastar | `star` |
| 483 | ratafio | `a ko` |
| 483 | ravelino | `lino` |
| 485 | reciproka | `ni reciproka` |
| 486 | regalar | `regala` |
| 487 | regento | `uver` |
| 489 | rekta | `eo ma` |
| 489 | rekuperar | `rekuperar. (trans., de` |
| 489 | rekursar | `r ad` |
| 490 | relatar | `pri termini, muziko-soni, afecion` |
| 490 | relegar | `yuro-cienco, che la Romani antiqu` |
| 490 | remisar | `aludante la si` |
| 490 | remisar | `tomi di morbo` |
| 491 | renet-pomo | `akul` |
| 491 | reno | `duop` |
| 493 | repozar | `en o` |
| 493 | repulsar | `repulsa` |
| 494 | resipicencar | `resipicenca` |
| 494 | respirar | `respir` |
| 494 | respondar | `responda` |
| 494 | responsar | `responsa` |
| 495 | retraktar | `etraktar` |
| 496 | retro- | `retro` |
| 496 | reversionar | `a do` |
| 497 | revoltar | `revolta` |
| 499 | rinocero | `paki` |
| 500 | ritmo | `riodo` |
| 500 | rizo | `oryza` |
| 500 | rizo | `sativa` |
| 501 | rocheto | `a epi` |
| 501 | rokambolo | `alliu` |
| 501 | rokambolo | `corodon` |
| 501 | rolo | `o. -` |
| 503 | rotano | `e pal` |
| 503 | rotondo | `vest` |
| 504 | rozario | `sas d` |
| 504 | rudimento | `- III` |
| 505 | rumoro | `vort` |
| 506 | rusho | `ruaDe` |
| 507 | sagaca | `cern` |
| 508 | sakrifikar | `ifikar` |
| 508 | sakrumo | `anat.) Os` |
| 509 | saliar | `sali` |
| 509 | salika | `salik` |
| 509 | salikario | `lythrum` |
| 509 | salikario | `salicaria` |
| 509 | salmono | `salmo (salar` |
| 509 | salolo | `galolo` |
| 511 | sandro | `sendro` |
| 511 | santuario | `chef` |
| 511 | sapar | `sapar.-` |
| 512 | sardino | `clupea` |
| 512 | sardino | `sardina` |
| 513 | sarkino | `sarki` |
| 513 | sat(e) | `sat (e` |
| 513 | satrapo | `ia-epoke` |
| 514 | *seancar | `seancan` |
| 514 | saturnali | `epoke` |
| 516 | sekondar | `andi, precipue Britaniana` |
| 516 | sekundara | `parent-` |
| 517 | sempre | `a, s` |
| 518 | senco | `signifiko kon` |
| 518 | sensuala | `sensu` |
| 519 | separar | `sepa` |
| 519 | septimo | `kermo` |
| 519 | septimo | `muzik` |
| 519 | sequestrar | `equestrar` |
| 520 | serfo | `a si` |
| 523 | shakto | `nala` |
| 526 | sibarito | `rito` |
| 527 | signifikar | `gnifikar` |
| 527 | sika | `s ko` |
| 529 | simular | `yurocienco` |
| 531 | sinoptika | `fio pozitivista` |
| 532 | siringo | `lo k` |
| 532 | siropo | `armac` |
| 533 | skabioso | `skabios` |
| 534 | skaleno | `kaleno` |
| 534 | skalio | `kalio` |
| 535 | skarmuchar | `aludante detach` |
| 535 | skarmuchar | `mento, tiralieri di du armei` |
| 535 | skeptika | `keptika` |
| 535 | skermar | `kermar` |
| 536 | skismo | `skism` |
| 536 | skisto | `ipua` |
| 536 | skolastiko | `kolastiko` |
| 536 | skolio | `skoli` |
| 537 | skotisho | `imil` |
| 537 | skrachar | `skracha` |
| 538 | skunero | `ro. (navig` |
| 538 | skurelo | `ilio` |
| 538 | skurelo | `rascurius` |
| 538 | slango | `anke` |
| 538 | slango | `extr` |
| 538 | slango | `rapti` |
| 538 | slingo | `irkum` |
| 538 | slingo | `lite` |
| 538 | sluzo | `inkl` |
| 539 | smeraldo | `iafana` |
| 539 | snapar | `snapa` |
| 539 | sobra | `meta` |
| 539 | sobra | `sobr` |
| 539 | socio | `la so` |
| 539 | socio | `soci` |
| 539 | sociologio | `la ci` |
| 539 | sociologo | `sociolog` |
| 539 | sodo | `famil` |
| 539 | sofisto | `epoki antiq` |
| 540 | sofito | `plaf` |
| 540 | soleo | `pleuronectes` |
| 540 | soleo | `solea` |
| 541 | solida | `as y` |
| 541 | somnambula | `rala` |
| 542 | sordida | `rdida` |
| 542 | sorgumo | `sorgum` |
| 544 | spatulo | `spatul` |
| 544 | spegulo | `spegul` |
| 544 | spektaklo | `jo t` |
| 544 | spektaklo | `pektaklo` |
| 544 | spektar | `ektar` |
| 544 | spektroskopio | `ektroskopio` |
| 544 | spektroskopo | `ektroskopo` |
| 545 | spermaceto | `halo` |
| 545 | spermocentro | `permocentro` |
| 546 | spiremo | `piremo` |
| 546 | spirito | `terio` |
| 547 | splisar | `splis` |
| 547 | sporangio | `sporangio.(bot` |
| 549 | stablo | `zajo` |
| 551 | stelario | `tellaria` |
| 553 | stigmato | `eligio kristana` |
| 553 | stilistiko | `listiko` |
| 554 | stiptika | `stipti` |
| 554 | stipular | `stipu` |
| 555 | storaxo | `raxo` |
| 555 | strangular | `metef` |
| 556 | strikta | `asas` |
| 556 | stroncio | `troncio` |
| 557 | studiar | `coze` |
| 557 | stupida | `a la` |
| 557 | sturgo | `aci-` |
| 557 | sturgo | `penser sturio` |
| 558 | sublima | `a, p` |
| 559 | subrekargo | `subrek` |
| 559 | substantivo | `adjek-` |
| 559 | substantivo | `tivo` |
| 559 | substitucar | `subst` |
| 559 | substitucar | `tucar` |
| 561 | sumnar | `trans, pri` |
| 564 | surtuto | `surtut` |
| 566 | takigrafar | `istem` |
| 568 | tampono | `s tr` |
| 569 | tapeto | `iguri` |
| 569 | tapiro | `rika` |
| 571 | tartaro | `tartar` |
| 572 | teatro | `reze` |
| 572 | teknologio | `tekenllogio` |
| 573 | temato | `tema` |
| 574 | tempestar | `aludante la elementi atmosferala quaz` |
| 574 | tenaca | `te a` |
| 575 | teogonio | `teogo` |
| 577 | termi | `suro` |
| 577 | termino | `atem` |
| 578 | termometro | `rmometro` |
| 581 | timbro | `timbalo` |
| 582 | tinlauro | `vibur` |
| 582 | tirozimazo | `russu-` |
| 583 | titrar | `emio` |
| 584 | tono | `mana` |
| 585 | topinamburo | `halianthus` |
| 585 | topinamburo | `tuberosus` |
| 585 | topologio | `ri es` |
| 585 | torako | `o e l` |
| 589 | trancar | `tran` |
| 589 | tranchar | `kuta` |
| 589 | trancheo | `ximi` |
| 590 | transmisar | `rgano` |
| 591 | traveo | `arkitakt` |
| 592 | trepidar | `aludante navo, automobilo, qua fun-` |
| 592 | trepidar | `cionas` |
| 593 | trezoro | `yuro-` |
| 593 | tribono | `ribono` |
| 594 | trikino | `ralis` |
| 594 | trikino | `trichinella spi-` |
| 598 | trumpeto | `r la` |
| 599 | tuberoso | `anthus tuberosa` |
| 599 | tuberoso | `poly-` |
| 600 | tumultar | `gran` |
| 600 | turgecar | `aludante ula organi, preci-` |
| 600 | turgecar | `pue le sexuala` |
| 603 | ultramikroskopio | `nsio` |
| 603 | unco | `felis` |
| 603 | unco | `onca` |
| 604 | unikursala | `unikursal` |
| 604 | urano | `or la` |
| 605 | urso | `vida` |
| 605 | urtiko | `a la` |
| 606 | -uyo | `- uyo` |
| 610 | vazo | `iqui` |
| 611 | vektoro | `esas` |
| 611 | velino | `a e p` |
| 612 | Venus | `che la Lati` |
| 613 | verdikto | `verdi` |
| 613 | verifikar | `u fa` |
| 614 | vertijar | `rtijar` |
| 615 | veziko | `anim` |
| 615 | viadukto | `arka` |
| 615 | vibriono | `ibriono` |
| 616 | viktimo | `tafore` |
| 622 | votar | `majorit` |
| 622 | vulnerario | `rario` |
| 622 | vulnerario | `vuln` |
| 622 | vulvo | `vulvo. (anat` |
| 623 | wolframo | `wolfra` |
| 625 | ye | `esas da la senco` |
| 626 | yuro | `ar (o` |
| 628 | zuavo | `po d` |
| 629 | cereala | `anat` |
| 629 | cereala | `cerebelo` |
| 629 | cerebelo | `cereala` |
| 629 | delto | `o, p` |

