...
TOP_CACHE = 24.0        # PostScript points; the block starts lower

def measure_(pdf, step_per_inch=0.1, orig_mm=21.9):
    step=step_per_inch*72; orig=orig_mm/25.4*72
    ...
    for ipg,pg in enumerate(re.split(r'<page ',xml)[1:]):
        for m in re.finditer(r'<word xMin="([\d.]+)" yMin="([\d.]+)" xMax="([\d.]+)" yMax="([\d.]+)">(.*?)</word>', pg):
            ...
            if not t: continue
            if float(m.group(2)) < TOP_CACHE: continue   # hidden mark
            u=(x-orig)/step
