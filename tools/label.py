...
def run_step(rounds=3):
    ...
    for round_ in range(rounds):
        ...
        rng=np.random.default_rng(round_); idx=[];l2=[]
        sure_ones=np.where(c>0.90)[0]
        per=max(1, min(8, 80000//max(len(sure_ones),1)))
        for k in sure_ones:
        ...
        print(f"  round {round_}: {len(sure_ones)} sure groups, {len(idx)} cells", flush=True)
        ...
        del X2,Y2,w
        print(f"  tour {round_} appris", flush=True)
