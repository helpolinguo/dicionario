...
from cells import underlines, refine_step
T=_ROOT + "/work"
def un(n):
    try:
        z=dict(np.load(f"{T}/cells/{n}.npz", allow_pickle=True))
        d=analyse(_ROOT + f"/scan/{n}.jpg")
        hstep, xg = refine_step(d['norm'], d['bloc'], d['pash'], d['xg'])
        xg = xg % hstep
        ...
        np.savez_compressed(f"{T}/cells/{n}.npz", **z)
        return n,"ok"
    except Exception as e:
        return n, f"ERR {type(e).__name__}: {e}"
if __name__=="__main__":
    names=[os.path.basename(p)[:-4] for p in sorted(glob.glob(f"{T}/cells/*.npz"))]
    with Pool(2) as p:
