...
from couverture import binariser_trait, SUR
from reparer_couverture import appliquer
...
def etapes():
    c=f"{RAC}/travail/couv/etapes.npz"
    if os.path.exists(c):
        z=np.load(c); return z['b'],z['d1'],z['d2']
    n=np.load(f"{RAC}/travail/couv/niveaux.npy")
    ...
    g=np.nonzero(ai>=GROS)[0]; Y0,Y1,X0,X1=bb[:,0],bb[:,1],bb[:,2],bb[:,3]
    for i,o in enumerate(ob):
        ...
        if len(g):
            ...
            if np.hypot(dy,dx).min()<=LOIN*SUR: continue
        d2[o] &= ~(l[o]==i+1)
...
def planche(nom,x0,x1,y0,y1,Z=5):
    ...
    for t,A in (("BINARISATION",b),("APRES DEPOUSS. 1",d1),("APRES DEPOUSS. 2",d2)):
        vues.append((t, (~A[y0*SUR:y1*SUR, x0*SUR:x1*SUR]*255).astype(np.uint8)))
    pl=Image.new('L',(W,(H+22)*len(vues)),255); d=ImageDraw.Draw(pl)
    ...
    os.makedirs(f"{RAC}/travail/audit",exist_ok=True)
    pl.save(f"{RAC}/travail/audit/etapes-{nom}.png"); print(nom,pl.size)
