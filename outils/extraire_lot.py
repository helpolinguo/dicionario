...
SCAN="/root/dicionario/scan"; OUT="/root/dicionario/travail/cellules"
os.makedirs(OUT, exist_ok=True)
def un(n):
    dst=f"{OUT}/{n}.npz"
    if os.path.exists(dst): return n,"deja"
    try:
        d=extraire(f"{SCAN}/{n}.jpg")
...
if __name__=="__main__":
    noms=sys.argv[1:]
    if not noms: noms=[os.path.basename(p)[:-4] for p in sorted(glob.glob(SCAN+"/*.jpg"))]
    with Pool(2) as pool:
        for i,(n,s) in enumerate(pool.imap_unordered(un, noms, chunksize=4)):
